...
def padding_inputs_width(image: tf.Tensor, target_shape: Tuple[int, int], increment: int) -> Tuple[
    ...
    def pad_fn():
        with tf.name_scope('mirror_padding'):
            ...
            padded_image = tf.pad(img_resized, padding, mode='SYMMETRIC', name=None)

            # Set manually the shape
            padded_image.set_shape([target_shape[0], target_shape[1], img_resized.get_shape()[2]])

            return padded_image, (new_h, new_w)
    ...
    def replicate_fn():
        with tf.name_scope('replication_padding'):
            ...
            padded_image = tf.image.crop_to_bounding_box(image=img_replicated, offset_height=0, offset_width=0,
